...
// ----------------------------------------------------------------------------
  ...
  Samantha.Router.prototype = $.extend({}, Samantha.Object.prototype, {
    ...
    listenForRouteTriggers: function() {
      var router = this;
        ...
        if (e.type === 'click') {
          ...
          switch(target.nodeName.toUpperCase())
          {
            ...
            //   return true;
					...
					// e.type === 'keypress'
					// note: input type=button will send a keypress
	
					if (e.target.nodeName.toUpperCase()=='INPUT' && e.target.type.toLowerCase() == 'button' && e.keyCode == 13) {
						$form = $target.closest('form');
            verb = $form.attr('method').toString().toLowerCase();
            path = $form.attr('action');
						routeFound = router.lookupRoute(verb, path);
